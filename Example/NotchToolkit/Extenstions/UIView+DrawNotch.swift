...
public extension UIView {
    /**
     **UIView.draw(notch type,position,curve,customBounds)** is a UIView extension that allows you add ovals and rounded corners to a UIView.
     - For type `oval`, set `curve` from 1.0 - 10.0.
     - For type `corner`, `curve` is the radius size.
     - Check `curveType` & `curvePosition` for more info.
     */
    public func draw(_ notch:curveType, position:curvePosition, curve:CGFloat?, customBounds:CGRect? = nil) {
        ...
        case .corner:
            ...
            case .bottom:
                ...
                break
            case .diagonalAC:
                viewPath = UIBezierPath(roundedRect: bounds, byRoundingCorners: [.topLeft, .bottomRight], cornerRadii: CGSize(width:curve!, height:curve!))
                break
            case .diagonalBD:
                viewPath = UIBezierPath(roundedRect: bounds, byRoundingCorners: [.topRight, .bottomLeft], cornerRadii: CGSize(width:curve!, height:curve!))
                break
            case .all:
